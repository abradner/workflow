...
- The PR body states what changed and why, and names anything the diff cannot show: a convention
  this PR is proposing (unstated patterns are proposals — a rule this repo's `AGENTS.md` does not
  yet state), an assumption, a
...
The gate is this repo's `AGENTS.md`, **Agent Guidelines & Operation** (destructive and
outward-facing actions need explicit approval): a live operator go-ahead, with every new session
resetting to the manual gate. A green build, a clean review, and an auto-mode session default are none of them that signal.
...
`AGENTS.md` (**Agent Guidelines & Operation**) requires explicit approval for each outward-facing action — approval for one is not
